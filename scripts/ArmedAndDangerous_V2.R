...
# Define a better marginal effects calculation function that works with endorse output
calculate_marginal_effects <- function(model_out, covariate_name, values = c(-2, -1, 0, 1, 2)) {
  # Extract posterior samples for the specific covariate
  posterior_samples <- model_out$delta
  
  # Check if the covariate is in the posterior samples
  if(!(covariate_name %in% colnames(posterior_samples))) {
    stop(paste("Covariate", covariate_name, "not found in posterior samples."))
  }
  
  # Get the coefficient estimates for the covariate
  coef_samples <- posterior_samples[, covariate_name]
  
  # Initialize results list for each question (endorser)
  results <- list()
  
  # Compute probabilities for each question (assuming 3 questions based on original code)
  for(q in 1:3) {
    # Initialize matrix to store probabilities
    probs <- matrix(NA, nrow = length(values), ncol = 3)  # for 2.5%, 50%, 97.5% quantiles
    
    # Calculate probabilities for each standardized value
    for(i in 1:length(values)) {
      # Calculate the effect size (standardized covariate value * coefficient)
      effect_sizes <- values[i] * coef_samples
      
      # Transform to probability scale (using inverse logit function)
      probs_samples <- 1 / (1 + exp(-effect_sizes))
      
      # Store quantiles for this value
      probs[i, ] <- quantile(probs_samples, probs = c(0.025, 0.5, 0.975))
    }
